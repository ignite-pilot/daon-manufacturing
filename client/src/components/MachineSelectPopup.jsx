...
/**
 * 사용 기계 선택 팝업 (이중 목록: 왼쪽 기계 목록 ↔ 오른쪽 사용 기계)
 * 왼쪽에서 선택 후 이동 → 사용 기계에 추가, 오른쪽에서 선택 후 이동 → 취소(제거)
 * 사용 부품 선택 화면과 동일한 UI/UX
 */
const toId = (v) => (v === null || v === undefined ? NaN : Number(v));

export default function MachineSelectPopup({ machines = [], selectedIds = [], onConfirm, onClose }) {
  ...
  const availableMachines = useMemo(
    () => filteredMachines.filter((m) => !localSelected.has(toId(m.id))),
    [filteredMachines, localSelected]
  );
  const selectedMachines = useMemo(
    () => machines.filter((m) => localSelected.has(toId(m.id))).sort((a, b) => toId(a.id) - toId(b.id)),
    [machines, localSelected]
  );
  ...
  return (
    <LayerPopup title="사용 기계 선택" onClose={onClose} maxWidth={640}>
      <div className="part-select-popup min-w-[560px]">
        ...
        <div className="flex gap-3 items-stretch h-[300px] part-select-popup-body">
            ...
            <ul className="flex-1 min-h-0 overflow-y-auto bg-white list-none">
              {availableMachines.length === 0 ? (
                <li className="p-3 text-gray-500 text-sm">
                  {searchQuery.trim() ? '검색 결과 없음' : '추가할 기계 없음'}
                </li>
              ) : (
                availableMachines.map((m) => {
                  const isSelected = leftSelection.has(toId(m.id));
                  return (
                    <li
                      key={m.id}
          ...
          <div className="flex-1 flex flex-col border border rounded-lg overflow-hidden min-w-0">
            <div className="px-2 py-1.5 bg-gray-100 border-b border-gray-200 text-sm font-medium text-gray-700 shrink-0">
              사용 기계 {selectedMachines.length > 0 && <span className="text-green-600">({selectedMachines.length}개 선택됨)</span>}
            </div>
            <ul className="flex-1 min-h-0 overflow-y-auto bg-white list-none">
              {selectedMachines.length === 0 ? (
                <li className="p-3 text-gray-500 text-sm">선택된 기계 없음</li>
              ) : (
                selectedMachines.map((m) => {
                  const isSelected = rightSelection.has(toId(m.id));
                  return (
                    <li
                      key={m.id}
        ...
        <p className="text-xs text-gray-500 mt-2">
          왼쪽에서 기계를 선택한 뒤 → 로 추가, 오른쪽에서 선택한 뒤 ← 로 제거
        </p>
